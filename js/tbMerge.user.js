        ...
        init:function(){
            ...
            if(tbMerge.isEmpty(strRegex)){
                ...
                if(contentHTML.match(isAgreed)){
                    contentHTML = contentHTML.replace(/是否已与各吧吧主/, '<span class="tb_agree green">[通过]</span>是否已与各吧吧主');
                    tbMerge.isDebug?console.log("吧主同意"):null;
                    $('.d_post_content:eq(0)').html(contentHTML);
                }else{
                    if(contentHTML.match(/是否已与各吧吧主/)){
                        contentHTML = contentHTML.replace(/是否已与各吧吧主/, '<span class="tb_agree red">[未通过]</span>是否已与各吧吧主');
                        $('.d_post_content:eq(0)').html(contentHTML);
                        tbMerge.isDebug?console.log("吧主拒绝"):null;
                    }else{
                        $("#errTips").show();
                    }
                }
                if(contentHTML.match(isMoved)){
                    contentHTML = contentHTML.replace(/是否已经转移/, '<span class="tb_move green">[通过]</span>是否已经转移');
                    $('.d_post_content:eq(0)').html(contentHTML);
                    tbMerge.isDebug?console.log("转移完成"):null;
                }else{
                    if(contentHTML.match(/是否已经转移/)){
                        contentHTML = contentHTML.replace(/是否已经转移/, '<span class="tb_move red">[未通过]</span>是否已经转移');
                        $('.d_post_content:eq(0)').html(contentHTML);
                        tbMerge.isDebug?console.log("转移拒绝"):null;
                    }else{
                        $("#errTips").show();
                    }
                }
